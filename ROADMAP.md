...
| M3-B3 | IndexedDB 自動儲存、還原、Decision Set 匯入／匯出與復原 | 開發分支本機實作；合併前仍須 Linux 視覺基準與遠端 CI |

M3-B3 開發分支已加入本機儲存，尚未提供完整工作流程編輯、Mock 或執行。下列為整個 M3 的交付與離開條件，不能因審查工作區通過而宣告整個 M3 完成。
